...
**重要实现细节**（2026-01-11 更新）：

*   **数据源分组架构**: 当前使用 `SourceGroupManager` 管理 `config/source_groups.json`，而不是旧的 `SourceManager`。所有数据源 API（GET/POST/PUT/DELETE /api/sources）都已适配这个新架构。
    *   删除操作会立即保存到配置文件
    *   后台调度器会自动停止抓取已删除的数据源
    *   不需要重启服务即可生效

*   **错误追踪集成**: 删除数据源时会自动清除相关的错误日志，避免已删除数据源的遗留错误信息污染错误追踪系统。
