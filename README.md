...
> **当前实现状态**：✅ **RSS 订阅** 已完全实现 | 🚧 **网站抓取、Twitter、本地文件** 数据源正在开发中
...
*   **多种数据源**（规划中）：
    *   ✅ **RSS 订阅**：支持标准 RSS/Atom feeds（已实现）
    *   🚧 **网站内容抓取**：通过 CSS 选择器抓取任意网站内容（开发中）
    *   🚧 **Twitter/X 追踪**：追踪指定账号的最新推文（开发中）
    *   🚧 **本地文件**：监听本地 Markdown 和 TXT 文件变化（开发中）
